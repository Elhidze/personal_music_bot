...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        if data is None:
            raise YTDLError('Sorry! Found no matches for `{}`(((((('.format(search))
        ...
        if 'entries' not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise YTDLError('N0 matches for `{}`(((('.format(webpage_url))
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='СМОТРИ, ВОТ ЭТО ЩАС ИГРАЕТ РЕАЛЬНО!!!!',
                               description='```css\n{0.source.title}\n```'.format(self),
                 ...
                 .set_footer(text=f'| by: kill me, elhidze#6666')
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('Ты чего, так нельзя в ЛС!')
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.send('ОШИБКА ОШИБКА ОШИБКА: {}'.format(str(error)))
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('А куда заходить????')
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send('Дурак? Бот выключен, зачем его стопать???')
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Музыка не играет, можешь включить!!!')
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Чел, сейчас музыка не играет, зачем её пропускать? Можешь включить!')
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('Сначала в войс зайди.')

        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                raise commands.CommandError('Бот уже в войсе!.')


bot = commands.Bot('#', description='Крутой бот так то, можешь чекнуть Elhidze, он сделал))).')
